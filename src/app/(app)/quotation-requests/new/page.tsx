...
export default function NewQuotationRequestPage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      title: '',  // ADDED: Initialize with empty string
      description: '',  // ADDED: Initialize with empty string
      isPublic: true,
  ...
  useEffect(() => {
    ...
    if (storedData && Object.keys(storedData).length > 0) {
        // FIXED: Ensure all fields have proper default values when restoring
        const sanitizedData = {
            title: storedData.title ?? '',
            description: storedData.description ?? '',
            isPublic: storedData.isPublic ?? true,
            companyIds: storedData.companyIds ?? [],
            industries: storedData.industries ?? [],
            dueDate: storedData.dueDate ? new Date(storedData.dueDate) : addDays(new Date(), 7),
            items: (storedData.items || [{ productName: '', description: '', quantity: 1 }]).map((item: any) => ({
